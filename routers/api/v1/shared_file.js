...
async function get_Router_callback(req, res)
{
  ...
  const QUERY_RESULT_SHARE_LINKS = await Firebase_Api.query_To_Database("share_links", [["where", "file_share_id", "==", FILE_SHARE_ID]])
  ...
  const QUERY_RESULT_FILE_INFOS = await Firebase_Api.query_To_Database("file_meta_datas", [["where", "file_uuid", "==", FILE_UUID]])
  if(QUERY_RESULT_FILE_INFOS.length == 0) throw new Error("The file meta datas is not searched !")
  const FILE_OWNER = QUERY_RESULT_FILE_INFOS[0].owner

  const QUERY_RESULT_SHARE_AUTHS = await Firebase_Api.query_To_Database("share_auths", [["where", "file_uuid", "==", FILE_UUID]])
  const FILE_SHARE_AUTHS = QUERY_RESULT_SHARE_AUTHS.map((doc_result) => doc_result.email_auth)

  if(!(USER_AUTH == FILE_OWNER || FILE_SHARE_AUTHS.includes(USER_AUTH)))
    throw new Error("The user auth is not suitable !")

  const DATA_URL = await Firebase_Api.string_data_From_Storage(`files/${FILE_UUID}`)
